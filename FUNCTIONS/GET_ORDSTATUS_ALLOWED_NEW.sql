--------------------------------------------------------
--  DDL for Function GET_ORDSTATUS_ALLOWED_NEW
--------------------------------------------------------

  CREATE OR REPLACE FUNCTION "YYORDERDB2"."GET_ORDSTATUS_ALLOWED_NEW" (pUSERNAME           IN VARCHAR2,
                                                             pORGANIZATION_ID   IN NUMBER,
                                                             pORD_STATUS         IN VARCHAR2,
                                                             pORD_TYPE           IN VARCHAR2,
                                                             pORD_MARKT          IN VARCHAR2,
                                                             pFL_INIT            IN VARCHAR2,
                                                             pFL_TRADE           IN VARCHAR2,
                                                             pFL_ADMIN           IN VARCHAR2)
...
BEGIN   
   -- Fall 3.5.6 "Loschen" wird uber die App geregelt und zusatzlich uber Trigger
      IF pORD_STATUS IS NULL AND pFL_INIT = 'J'
      ...
      IF pORD_STATUS IN (AOO.Stat_Sleeping) AND pFL_INIT = 'J'
      -- Fall 3.5.1 bis 3.5.5
      ...
      IF pORD_STATUS IN (AOO.Stat_New) AND pFL_INIT = 'J'
      ...
      IF pORD_STATUS IN (AOO.Stat_Deletion_Request) AND pFL_TRADE = 'J'
      -- Wenn der Datensatz angelegt wurde und der User das Init Recht hat, dann darf er ihn auf "DELETION REQUEST" setzen.
      THEN
         PIPE ROW (ordstatus_type (AOO.Stat_Deletion_Confirmed));
      END IF;

      IF pORD_STATUS IN (AOO.Stat_New, AOO.Stat_Update_Request) AND pFL_TRADE = 'J'
      ...
      IF pORD_STATUS IN (AOO.Stat_In_System, AOO.Stat_Await_Settle, AOO.Stat_Update_Request) AND pFL_INIT = 'J'
      ...
      IF pORD_STATUS IN (AOO.Stat_Executed) AND pFL_ADMIN = 'J'
      ...
      IF pORD_STATUS IN (AOO.Stat_Cancellation_Request) AND pFL_TRADE = 'J'
      -- Fall 3.5.7
      ...
      IF pORD_STATUS IN (AOO.Stat_In_System) AND pFL_TRADE = 'J'
      -- Fall 3.5.9
...
EXCEPTION
   ...
   THEN
         ...
         logger.log_permanent (FORMATS('Fehler! {0} - {1}', VARARGS(SQLCODE, SQLERRM)));
END;
